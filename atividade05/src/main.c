...
if(rank == 0){
    ...
    for(int i = 1; i < tasks_qtd; i++){
        ...
        MPI_Send(mat2, dimMat2[0]*dimMat2[1], MPI_FLOAT, i, 1, MPI_COMM_WORLD);
    }
    ...
    for(int i = 1; i < tasks_qtd; i++){
        ...
        MPI_Send(mat1 + i*(dimSubMat1[0]*dimSubMat1[1]), dimSubMat1[0]*dimSubMat1[1], MPI_FLOAT, i, 3, MPI_COMM_WORLD);
    }
    ...
    MPI_Recv(b_mat2, dimMat2[0]*dimMat2[1], MPI_FLOAT, 0, 1, MPI_COMM_WORLD, &status);
    ...
    MPI_Recv(b_mat1, dimSubMat1[0]*dimSubMat1[1], MPI_FLOAT, 0, 3, MPI_COMM_WORLD, &status);
...
if(rank == 0){
    ...
    MPI_Send(b_result, dimSubMat1[0]*dimMat2[1], MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
